...
class image_mdl_image extends dbeav_model{
    ...
    function store($file,$image_id,$size=null,$name=null,$watermark=false){
        ...
        list($w,$h,$t) = getimagesize($file)?getimagesize($file):array(16,16,17);
        ...
        //自定义的情况走新情况
        $image_size=$this->app->getConf('image.size');
        unset($image_size['L']);
        unset($image_size['M']);
        unset($image_size['S']);
        $image_size=array_keys((array)$image_size);
        if(in_array(strtoupper($size), (array)$image_size)){
            foreach(kernel::servicelist('image.other.action') as $object){
                if(is_object($object) && method_exists($object,'save_other_size')){
                    $object->save_other_size($params,$size);
                }
            }
        }else{
            parent::save($params);
    ...
    function rebuild($image_id,$sizes,$watermark=true){
        ...
        foreach(kernel::servicelist('image.other.action') as $object){
            if(is_object($object) && method_exists($object,'get_other_size')){
                $object->get_other_size($sizes);
            }
        }
        ...
        if($sizes){
            ...
            if(ECAE_MODE){
                if($img['s_ident']){
                    ecae_file_delete($img['s_ident']);
                }
                if($img['m_ident']){
                    ecae_file_delete($img['m_ident']);
                }
                if($img['l_ident']){
                    ecae_file_delete($img['l_ident']);
                }
            }
            */
           foreach($sizes as $s){
    ...
    */
	...
	public function delete($filter){
        $sql = 'DELETE FROM sdb_image_image  where '.$this->filter($filter);
