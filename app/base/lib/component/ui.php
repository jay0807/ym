...
class base_component_ui {
    ...
    function __construct($controller, $app_specal=null){
        ...
        if( $app_specal){
            ...
            if(defined('HOST_MIRRORS')&&$_SERVER['HTTPS'] != 'on' && $_SERVER['SERVER_PORT']!='443'){
                    ...
                    if(is_array($host_mirrors) && isset($host_mirrors[0])){
                        ...
                        $this->host_mirrors = $host_mirrors[rand(0,$host_mirrors_count)];
                    }
    ...
    function img($params){
        ...
        if(is_array($this->_imgbundle[$app->app_id]['info']) && array_key_exists($params['src'], $this->_imgbundle[$app->app_id]['info'])){
            ...
            $params['src'] = $this->host_mirrors.app::get('base')->res_url.'/transparent.gif';
            ...
            $params['src'] = $this->host_mirrors.$app->res_url.'/'.$params['src'];
        }
        unset($params['lib']);
...
EOF;
    ...
    function script($params){
        ...
        if($params['content']){
            ...
            return '<script type="text/javascript" src="'.$this->host_mirrors.$app->res_url.'/'.$pdir.'/'.$params['src'].'?'.$time.'"></script>';
        }
    }
    ...
    function css($params)
    {
		...
		if (is_dir($app->res_dir.'/'.$pdir))
			$file = $this->host_mirrors.$app->res_url.'/'.$pdir.'/'.$params['src'];
		else
			$file = $this->host_mirrors.$app->res_url.'/'.$params['src'];
...
EOF;
    ...
    function lang_script($params){
        ...
        if($params['content']){
            ...
            $time = substr(cachemgr::ask_cache_check_version(), 0, 6);
            return '<script type="text/javascript" src="'.$this->host_mirrors.$app->lang_url.'/'.$lang.'/'.$src.'?'.$time.'"></script>';
        }
    }
