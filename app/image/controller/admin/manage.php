...
class image_ctl_admin_manage extends desktop_controller 
{
    ...
    function image_upload(){
       $file_ext = substr($_FILES['upload_item']['name'], -4);
       if($_POST['image_input_type']=='favicon' && $_FILES['upload_item']['type']!='image/x-icon' && $file_ext!='.ico'){
       	   header('Content-Type:text/html; charset=utf-8');
           echo "{error:'".app::get('image')->_('favicon图片仅支持.ico格式')."',splash:'true'}";
           exit;
       }
       $mdl_img   = $this->app->model('image');
...
EOF;
    ...
    function image_broswer($page=1){
		...
		if($_GET['image_input_type']){
			$this->pagedata['image_input_type'] = $_GET['image_input_type'];
		}
...
EOF;
    ...
    function validImageSiez($pic){
        $def_image_set = app::get('image')->getConf('image.default.set');
        $tem_size = array();
        foreach($pic as $k=>$v){
            if(!empty($v['width']) && !empty($v['height'])){
                $tem_size[$k] = $v['width'].'-'.$v['height'];
            }else{
                $tem_size[$k] = $def_image_set[$k]['width'].'-'.$def_image_set[$k]['height'];
            }
        }
        if(count($tem_size) != count(array_unique($tem_size))){
            $this->splash('error','',app::get('image')->_('操作失败！存在尺寸一样的图片规格'));
        }
    }
    /**
     * 图片大小配置
    ...
    function imageset(){
       ...
       $allsize = array();
        if($_POST['pic']){
            //验证上传图片规格有没重复 by zhoulei 2012-5-25
            $this->validImageSiez($_POST['pic']);
            ...
            foreach($image_set as $size=>$item){
                ...
                $img = $image->dump($item['default_image']);
                ...
                if(substr($org_file,0,4)=='http'){
                    ...
                    if($img['storage']=='network'){
                        ...
                        if($response===false){
                            $data = array('image_id'=>$item['default_image'],'last_modified'=>time());
                            $image->save($data);
                            return true;                    
                        }
                ...
                image_clip::image_resize($image,$org_file,$tmp_target,$item['width'],$item['height']);
                $image_id=$image->store($tmp_target);
                $image_set[$size]['default_image'] = $image_id;
            }
        ...
        $cur_image_set = $this->app->getConf('image.set');
        foreach($cur_image_set as $size=>$value){
            $image_size[$size]='('.$value['width'].'*'.$value['height'].')';
        }

        //设置上传图片的自定义规格
        $def_image_set = app::get('image')->getConf('image.default.set');
        $def_image_set['X']=$def_image_set['L'];
        $def_image_set['Y']=$def_image_set['L'];
        $def_image_set['Z']=$def_image_set['L'];
        $def_image_set['X']['title']='自定义设置一';
        $def_image_set['Y']['title']='自定义设置二';
        $def_image_set['Z']['title']='自定义设置三';
        app::get('image')->setConf('image.default.set',$def_image_set);
        
        $this->app->setConf('image.size',$image_size);
